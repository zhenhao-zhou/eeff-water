...
#define QUADRATIC_TOL 0.2
#define EMACH 1.0e-10
...
int MinLineSearch::linemin_backtrack(double eoriginal, double &alpha)
{
  ...
  // 1. 计算搜索方向在全局梯度上的投影（判定是否沿负梯度方向）
  fdothme = 0.0;
  int me;
  MPI_Comm_rank(world, &me);
  //原子坐标自由度数目*3（即系统的位移自由度维度）
  //h 通常表示当前位置沿某个方向进行线性搜索的方向。h[i] 是提前由最小化方法（如 cg, sd, fire 等）计算好的
  for (i = 0; i < nvec; i++) {
	  //utils::logmesg(lmp, "me,i,fvec[i], h[i] = {:d},{:d},{:.12f}, {:.12f}\n",me,i,fvec[i], h[i]);
	  fdothme += fvec[i]*h[i];
  }
  if (nextra_atom)
    for (m = 0; m < nextra_atom; m++) {
      ...
      for (i = 0; i < n; i++) {
		  ...
		  //utils::logmesg(lmp, "me,i,fatom[i], hatom[i] = {:d},{:d},{:.12f}, {:.12f}\n",me,i,fatom[i], hatom[i]);
	  }
	}
  //utils::logmesg(lmp, "before_sum: me, fdothme ={:d}, {:.16f}\n", me, fdothme);	
  MPI_Allreduce(&fdothme,&fdothall,1,MPI_DOUBLE,MPI_SUM,world);
  if (nextra_global)
    for (i = 0; i < nextra_global; i++) fdothall += fextra[i]*hextra[i];
  //utils::logmesg(lmp, "fdothall = {:.12f}\n", fdothall);
  ...
  while (true) {
	...
	//utils::logmesg(lmp, "de_ideal, de = {:.14f}, {:.14f}\n", de_ideal, de);
    if (de <= de_ideal) {
      if (nextra_global) {
