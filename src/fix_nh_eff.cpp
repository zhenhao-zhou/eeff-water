...
void FixNHEff::nve_x()
{
  ...
  for (int i = 0; i < nlocal; i++)
    if (mask[i] & groupbit)
      if (abs(spin[i])>=1 && abs(spin[i])<=5) eradius[i] += dtv * ervel[i];
}
...
/*
  ...
  for (int i = 0; i < nlocal; i++)
    if (mask[i] & groupbit)
      if (abs(spin[i])==1 || abs(spin[i])==2) ervel[i] *= factor_eta;
  //fprintf(universe->uscreen,"eff_factor_eta:%d %f\n",universe->me, factor_eta);
  */
}
